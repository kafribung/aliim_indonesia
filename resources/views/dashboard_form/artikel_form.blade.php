...
<div class="form-group">
    ...
    <img src="{{ url($artikel->takeImg) }}" alt="Gambar {{$artikel->slug}}" title="{{$artikel->slug}}" width="100" height="100">
    @endif
...
<div class="form-group">
    ...
    <select id="kategori" name="kategori[]" class="form-control @error('kategori') is-invalid @enderror" required multiple>
        @if ($artikel->slug != null)
            <optgroup label="Old Kategori">
            @foreach ($artikel->kategori_artikels as $kategori)
            <option disabled class="font-weight-bold font-italic">{{$kategori->title}}</option>
            @endforeach
        @endif
        @foreach ($kategoris as $kategori)
        <option {{old('kategori') == $kategori->id ? 'selected' : ''}} value="{{$kategori->id}}"> {{$kategori->title}} </option>
        @endforeach
    </select>
    @error('kategori')
    <p class="alert alert-danger">{{$message}}</p>
