                ...
                <aside class="profile-nav alt">
                    <section class="card">
                        ...
                        <div class="d-flex justify-content-end">
                            <small>{{ $doa->user->name }} | </small>
                            <small>{{ $doa->created_at->diffForHumans() }}</small>
                        </div>
                        <hr>

                        @if ($doa->author())
                        <div class="card-text text-sm-center">
                            <a href="/doa-hadist/{{$doa->slug}}/edit" class="btn btn-outline-warning btn-sm">
                                <i class="fa fa-edit"></i>
                            </a>
                            <form action="/doa-hadist/{{$doa->id}}" method="POST" class="d-inline-flex">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Hapus Data {{$doa->title}} ?')"
